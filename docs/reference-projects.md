...
- `Huawei-To-Honor-Theme-Converter` / `exarch`：资源键和包名路径映射现在采用边界匹配；保留参考项目确认的替换规则，同时把普通子串排除在自动转换之外，并由回归测试固定该失败关闭边界。
- `Huawei-To-Honor-Theme-Converter` / `exarch`：源主题兼容性报告现在统计实际推导出的荣耀目标，单独标出 fan-out，并限制样本数量；映射可观测性与输入边界保持同一份报告。
