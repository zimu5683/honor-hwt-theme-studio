...
APK 只接受包含根目录 `description.xml` 文件的有效 ZIP，并在读取 ZIP 中心目录时限制条目数量不超过 20,000、单个条目不超过 256 MiB、所有条目累计解压量不超过 512 MiB；外层 ZIP 的本地文件数据区间物理重叠、文件/目录路径前缀重叠、规范化重复路径、路径穿越和超高压缩比也会在读取内容前拒绝；校验 SHA-256 后才以临时文件方式替换目标文件。成功返回：
